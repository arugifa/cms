...
class BaseFileReader(ABC, BaseCommandLine):
    ...
    def __call__(self, path: Union[str, Path]) -> 'FileOpener':
...
@dataclass
class FileOpener(AbstractAsyncContextManager):
    ...
    async def __aenter__(self) -> BaseFileReader:
